...
export class GrvtWebSocketServer {
  ...
  private onConnection(ws: WebSocket, req: IncomingMessage): void {
    ...
    if (providedToken && this.verifyToken) {
      ...
      userId = null;
    } else if (!providedKey && !providedToken && !this.apiKey) {
      // Self-hosted bypass: when no dashboard API key is configured anywhere,
      // allow anonymous WS connections (treated as operator, userId = null
      // so ownership checks are skipped — same as the legacy operator path).
      userId = null;
    } else {
