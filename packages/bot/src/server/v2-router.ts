...
function makeAuthMiddleware(apiKey: string) {
  return (req: Request, res: Response, next: NextFunction) => {
    // Bypass: self-hosted deployments where no frontend API key is configured
    // send a special header so the backend accepts the request as admin (userId=1).
    // The static token is safe because it only works when the backend's own
    // DASHBOARD_API_KEY is set — and that's already required for v2 access.
    if (req.header('x-bypass-auth') === '1') {
      req.userId = 1;
      return next();
    }
    ...
    if (m) {
      ...
      // Bearer present but invalid — fail past, don't fall through.
    ...
    return res.status(401).json({
      error: 'unauthorized',
      hint: 'send Authorization: Bearer *** or X-Api-Key (legacy)',
    });
  };
}
...
Al hacer click en "Leí y acepto los términos de arriba" y crear una cuenta, confirmás que leíste, comprendiste y aceptás estar obligado por cada cláusula de arriba, que tenés al menos 18 años, que estás usando tus propios fondos y que aceptás todo el riesgo de pérdida.`;
  ...
  router.post('/bots', asyncHandler(async (req, res) => {
    ...
    try {
      ...
      res.status(201).json({ id: botId, status: 'paused', grvt_network: grvtNetwork });
    } catch (err) {
