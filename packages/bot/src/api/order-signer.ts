...
// Prod = 325, testnet = 326. Keep configurable because using prod chainId
// against testnet causes GRVT to reject orders with "Signature does not match payload".
function getEip712Domain(chainId?: number) {
  return {
    name: 'GRVT Exchange',
    version: '0',
    chainId: chainId ?? Number(process.env.GRVT_EIP712_CHAIN_ID || (process.env.GRVT_ENV === 'testnet' ? 326 : 325)),
  };
}
...
function roundSizeToInstrument(size: string | number, instrument: string): string {
  const s = typeof size === 'string' ? parseFloat(size) : size;
  const minSize = getInstrumentSpec(instrument).min_size || 0.01;
  const rounded = Math.floor(s / minSize) * minSize;
  return rounded.toFixed(12).replace(/\.0+$/, '').replace(/(\.\d*?)0+$/, '$1');
}
...
export interface SigningCreds {
  ...
  chainId?: number;        // 326 testnet, 325 mainnet
}
...
export async function signOrder(
  ...
  const typedData = {
    ...
    domain: getEip712Domain(signingCreds?.chainId),
    types: EIP712_TYPES,
  ...
  console.log('\n🏢 EIP-712 domain:');
  console.log(JSON.stringify(typedData.domain, null, 2));
      ...
      legs: [
        {
          ...
          size: roundSizeToInstrument(size, instrument), // match signed contractSize/min_size exactly
