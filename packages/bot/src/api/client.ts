...
export function getGrvtEndpointConfig(network?: GrvtNetwork) {
  const resolved: GrvtNetwork = network ?? (process.env.GRVT_ENV === 'testnet' ? 'testnet' : 'mainnet');
  if (resolved === 'testnet') {
    return {
      network: resolved,
      edgeUrl: 'https://edge.testnet.grvt.io',
      marketDataUrl: 'https://market-data.testnet.grvt.io/full/v1',
      tradingUrl: 'https://trades.testnet.grvt.io/full/v1',
      eip712ChainId: 326,
    };
  }
  return {
    ...
    marketDataUrl: 'https://market-data.grvt.io/full/v1',
    tradingUrl: 'https://trades.grvt.io/full/v1',
    eip712ChainId: 325,
  };
}

const LEGACY_NETWORK: GrvtNetwork = process.env.GRVT_ENV === 'testnet' ? 'testnet' : 'mainnet';
const LEGACY_ENDPOINTS = {
  ...getGrvtEndpointConfig(LEGACY_NETWORK),
  edgeUrl: (process.env.GRVT_EDGE_URL || getGrvtEndpointConfig(LEGACY_NETWORK).edgeUrl).replace(/\/$/, ''),
  marketDataUrl: (process.env.GRVT_MARKET_DATA_URL || getGrvtEndpointConfig(LEGACY_NETWORK).marketDataUrl).replace(/\/$/, ''),
  tradingUrl: (process.env.GRVT_TRADING_URL || getGrvtEndpointConfig(LEGACY_NETWORK).tradingUrl).replace(/\/$/, ''),
};
...
export class GRVTClient {
  ...
  private network: GrvtNetwork;
  private edgeUrl: string;
  private marketDataUrl: string;
  private tradingUrl: string;
  private eip712ChainId: number;

  constructor(creds?: GrvtClientCreds, network?: GrvtNetwork) {
    this.instanceAuthState = createEmptyAuthState();
    this.creds = creds ?? null;
    const endpoints = creds || network ? getGrvtEndpointConfig(network ?? LEGACY_NETWORK) : LEGACY_ENDPOINTS;
    this.network = endpoints.network;
    this.edgeUrl = endpoints.edgeUrl.replace(/\/$/, '');
    this.marketDataUrl = endpoints.marketDataUrl.replace(/\/$/, '');
    this.tradingUrl = endpoints.tradingUrl.replace(/\/$/, '');
    this.eip712ChainId = endpoints.eip712ChainId;
  ...
  async login(): Promise<boolean> {
    if (this.creds) {
      return authenticateWithKey(this.creds.apiKey, this.instanceAuthState, this.edgeUrl);
    }
  ...
  private async authedRequest(url: string, body: object = {}, options?: { method?: string; timeout?: number }): Promise<any> {
    if (this.creds) {
      return authenticatedRequestWithState(this.instanceAuthState, this.creds.apiKey, url, body, { ...options, edgeUrl: this.edgeUrl });
    }
    return authenticatedRequest(url, body, options);
  }

  /** Get the signing credentials for this client (for order-signer). */
  getSigningCreds(): { privateKey: string; signerAddress: string; subAccountId: string; chainId: number } {
    if (this.creds) {
      return {
    ...
    return { privateKey, signerAddress, subAccountId, chainId: this.eip712ChainId };
  }
  ...
  async getTicker(instrument: string): Promise<Ticker> {
    const data = await publicRequest(`${this.marketDataUrl}/ticker`, {
      instrument
    });
    return data;
  }
  ...
  async getInstruments(): Promise<any[]> {
    const data = await publicRequest(`${this.marketDataUrl}/instruments`, {});
  ...
  async getKlines(
    ...
    const data = await publicRequest(`${this.marketDataUrl}/kline`, {
      instrument,
      interval,
      type: 'TRADE',
      limit
  ...
  async getBalance(): Promise<Balance> {
    ...
    const data = await this.authedRequest(`${this.tradingUrl}/account_summary`, {
  ...
  async getPositions(): Promise<Position[]> {
    ...
    const data = await this.authedRequest(`${this.tradingUrl}/positions`, { sub_account_id: this.tradingAccountId });
  ...
  async getOpenOrders(instrument?: string): Promise<Order[]> {
    ...
    const data = await this.authedRequest(`${this.tradingUrl}/open_orders`, body);
  ...
  async createOrder(request: CreateOrderRequest, allowMarket: boolean = false): Promise<Order> {
    ...
    try {
      ...
      const data = await this.authedRequest(`${this.tradingUrl}/create_order`, orderData);
  ...
  async cancelOrder(orderId: string, instrument: string): Promise<boolean> {
    ...
    try {
      await this.authedRequest(`${this.tradingUrl}/cancel_order`, {
        sub_account_id: this.tradingAccountId,
        order_id: orderId,
  ...
  async cancelAllOrders(instrument?: string): Promise<number> {
    ...
    try {
      const data = await this.authedRequest(`${this.tradingUrl}/cancel_all_orders`, body);
  ...
  async setLeverage(instrument: string, leverage: number): Promise<boolean> {
    ...
    try {
      await this.authedRequest(`${this.tradingUrl}/set_leverage`, {
        sub_account_id: this.tradingAccountId,
        instrument: instrument,
  ...
  async getFillHistory(
    ...
    const data = await this.authedRequest(`${this.tradingUrl}/fill_history`, body);
    return Array.isArray(data) ? data : [];
  }
