...
export async function authenticateGRVT(): Promise<boolean> {
  try {
    ...
    const response = await fetchIPv4(`${(process.env.GRVT_EDGE_URL || 'https://edge.grvt.io').replace(/\/$/, '')}/auth/api_key/login`, {
      method: 'POST',
      headers: {
...
export async function authenticateWithKey(
  ...
  try {
    const response = await fetchIPv4(`${(edgeUrl || process.env.GRVT_EDGE_URL || 'https://edge.grvt.io').replace(/\/$/, '')}/auth/api_key/login`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'User-Agent': 'GRVT-Grid-Bot/1.0',
...
export async function authenticatedRequestWithState(
  ...
  url: string,
  body: object = {},
  options: { method?: string; timeout?: number; edgeUrl?: string } = {}
): Promise<any> {
  // Re-auth if needed
  if (!state.isAuthenticated || (state.expiresAt - Date.now()) < 60 * 60 * 1000) {
    const ok = await authenticateWithKey(apiKey, state, options.edgeUrl);
