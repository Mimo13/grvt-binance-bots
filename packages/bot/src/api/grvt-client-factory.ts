...
import { GRVTClient, type GrvtClientCreds, type GrvtNetwork } from './client.js';
...
function cacheKey(userId: number, subAccountId: number | null, network: GrvtNetwork): string {
  return `${userId}:${subAccountId ?? 'default'}:${network}`;
}
...
export async function getGrvtClientForBot(
  ...
  gridBotDb: GridBotDB,
  network: GrvtNetwork = 'testnet'
): Promise<GRVTClient> {
  const key = cacheKey(userId, subAccountId, network);
  ...
  const client = new GRVTClient(creds, network);
...
export async function getGrvtClientForUser(
  ...
  return getGrvtClientForBot(userId, null, gridBotDb, 'testnet');
}
 ...
 */
export function invalidateGrvtClient(userId: number, subAccountId?: number | null, network?: GrvtNetwork): void {
  ...
  if (network) {
    cache.delete(cacheKey(userId, subAccountId, network));
    return;
  }
  for (const k of Array.from(cache.keys())) {
    if (k.startsWith(`${userId}:${subAccountId ?? 'default'}:`)) cache.delete(k);
  }
}
