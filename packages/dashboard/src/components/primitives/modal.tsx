...
interface ModalProps {
  ...
  title: string;
  description?: string;
  // 'wide' for wizards, 'regular' for confirms, 'fullscreen' for dense flows.
  size?: 'regular' | 'wide' | 'fullscreen';
  children: ReactNode;
  footer?: ReactNode;
}

const SIZE_CLASS: Record<'regular' | 'wide' | 'fullscreen', string> = {
  regular: 'max-w-[560px]',
  wide: 'max-w-[720px]',
  fullscreen: 'max-w-[1180px] md:w-[94vw]',
};

const HEIGHT_CLASS: Record<'regular' | 'wide' | 'fullscreen', string> = {
  regular: 'h-auto',
  wide: 'h-auto',
  // Dense wizard flows should not collapse to header/footer height on short
  // desktop viewports. Give them a real viewport-bound height and let the
  // body scroll inside the modal.
  fullscreen: 'h-[96dvh] md:h-[92dvh]',
};
  ...
  return (
    <dialog
      ...
      className={cn(
        ...
        'w-full max-h-[96dvh] md:max-h-[92dvh]',
