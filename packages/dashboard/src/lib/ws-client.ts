...
class WsClient {
  ...
  constructor() {
    ...
    // Bypass mode (no API key configured): omit auth param so the server
    // accepts the connection via its internal bypass (same as REST API).
    this.url = apiKey
      ? `${wsBase}/ws?api_key=${encodeURIComponent(apiKey)}`
      : `${wsBase}/ws`;
  }
