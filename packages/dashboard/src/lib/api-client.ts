...
// Dev/self-host bypass: when no API key is configured, skip sending
// auth headers and also skip JWT checks by sending a special header
// the backend will recognise.
const BYPASS_MODE = !LEGACY_API_KEY;
const BYPASS_HEADER = 'x-bypass-auth';
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  // Bypass mode: no auth needed (dev / self-hosted no-proxy setup)
  if (!BYPASS_MODE) {
    ...
    // Signal to the backend that it should accept this as a bypass.
